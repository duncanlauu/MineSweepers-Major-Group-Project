...
from app.recommender_system.books_recommender import get_top_n, get_top_n_for_genre, get_top_n_for_club, \
    ...
    get_global_top_between_m_and_n, get_global_top_between_m_and_n_for_genre
...
class RecommenderAPITestCase(APITestCase):
    ...
    def test_everything(self):
        ...
        self._get_top_between_m_and_n_for_club_test()
        self._get_top_between_m_and_n_for_club_for_genre_test()
        self._get_top_between_m_and_n_global_test()
        self._get_top_between_m_and_n_global_for_genre_test()
        self._get_top_between_m_and_n_users_top_books_test()
        self._get_top_between_m_and_n_users_random_books_test()
        self._get_top_between_m_and_n_users_genre_books_test()
        self._get_top_between_m_and_n_clubs_top_user_books_test()
        self._get_top_between_m_and_n_clubs_random_books_test()
        self._get_top_between_m_and_n_clubs_genre_books_test()
        self._get_top_between_m_and_n_clubs_top_club_books_test()

    def _post_top_n_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n'}
    ...
    def _post_top_n_for_genre_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n_for_genre', 'genre': self.genre}
    ...
    def _post_top_n_for_club_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.club.id, 'action': 'top_n_for_club'}
    ...
    def _post_top_n_for_club_for_genre_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.club.id, 'action': 'top_n_for_club_for_genre', 'genre': self.genre}
        pred_lookup = generate_pred_set(self.pred)
        url = reverse('app:recommender_top_n_for_genre', kwargs=args)
        num_of_recommendations_before = BookRecommendationForClub.objects.count()
        response = self.client.post(url)
    ...
    def _post_top_n_global_test(self):
        args = {'m': self.m, 'n': self.n, 'action': 'top_n_global'}
    ...
    def _post_top_n_global_for_genre_test(self):
        args = {'m': self.m, 'n': self.n, 'action': 'top_n_global_for_genre', 'genre': self.genre}
    ...
    def _post_top_n_users_top_books_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n_users_top_books'}
        url = reverse('app:recommender_top_n', kwargs=args)
    ...
    def _get_top_n_for_genre_test(self):
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n_for_genre', 'genre': 'fiction'}
        url = reverse('app:recommender_top_n_for_genre', kwargs=args)
        num_of_recommendations_before = BookRecommendation.objects.count()
        response = self.client.post(url)
        top_n = get_top_n_for_genre(self.uid, self.trainset, self.algo, self.genre, self.n)
    ...
    def _get_top_between_m_and_n_test(self):
        m = 5
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n'}
        url = reverse('app:recommender_top_n', kwargs=args)
        num_of_recommendations_before = BookRecommendation.objects.count()
    ...
    def _get_top_between_m_and_n_global_for_genre_test(self):
        m = 5
        args = {'m': self.m, 'n': self.n, 'action': 'top_n_global_for_genre', 'genre': self.genre}
        url = reverse('app:recommender_top_n_global_for_genre', kwargs=args)
        num_of_recommendations_before = GlobalBookRecommendation.objects.count()
    ...
    def _get_top_between_m_and_n_users_top_books_test(self):
        m = 5
        args = {'m': self.m, 'n': self.n, 'id': self.uid, 'action': 'top_n_users_top_books'}
        url = reverse('app:recommender_top_n', kwargs=args)
        num_of_recommendations_before = UserRecommendation.objects.count()
