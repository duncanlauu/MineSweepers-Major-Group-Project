            ...
            if serializer.is_valid():
                ...
                if new_meeting:
                    if 'book' not in request.data:
                        ...
                        new_meeting.book_id = Book.objects.get(title=request.data['book']).ISBN
...
class MeetingsView(APIView):
    """This is an API for getting all meetings for a user"""

    def get(self, request, *args, **kwargs):
        if 'id' not in kwargs:
            return Response(data='You need to provide the user id', status=status.HTTP_400_BAD_REQUEST)
        if kwargs['id'] != request.user.id:
            return Response(data='You can only see your own meetings', status=status.HTTP_401_UNAUTHORIZED)
        list_of_meetings = []
        for meeting in Meeting.objects.all():
            if kwargs['id'] in (user.id for user in meeting.attendees.all()):
